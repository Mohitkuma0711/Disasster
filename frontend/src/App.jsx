...
export default function App() {
  ...
  const [currentModule, setCurrentModule] = useState('victim-triage'); // 'victim-triage', 'climate-risk', 'video-analysis', or 'rescue-table'
  ...
  const handleVictimRescued = (victimId) => {
    setVictims(prev => prev.map(v => v.id === victimId ? { ...v, status: 'rescued', rescuedAt: new Date().toISOString() } : v));
  };
...
{/* Module Navigation Tabs */}
          <div className="flex items-center space-x-2 bg-slate-900/90 p-1.5 rounded-lg border border-slate-800">
            ...
            <button
              onClick={() => setCurrentModule('rescue-table')}
              className={`px-3.5 py-1.5 rounded text-xs font-mono font-bold transition flex items-center space-x-2 ${
                currentModule === 'rescue-table'
                  ? 'bg-amber-900/60 text-white border border-amber-600/60 shadow-lg'
                  : 'text-slate-400 hover:text-slate-200'
              }`}
            >
      ...
      <main className="max-w-7xl mx-auto px-4 mt-6 space-y-6">
        ...
        {currentModule === 'victim-triage' && (
          <>
            ...
            <PriorityQueue victims={victims} onVictimSelect={setSelectedVictim} onVictimRescued={handleVictimRescued} />
          </>
        )}
        ...
        {/* Module 4: Rescue Table */}
        {currentModule === 'rescue-table' && <RescueTable />}
